...
- **[ai-oncall](https://github.com/kalyvask/ai-oncall)**: LLM agent that diagnoses production incidents in under 30 seconds. Live connectors to Prometheus and Loki; service graph built from OTel spans; topology-based pruning of impossible hypotheses before the 8-call investigation loop; per-hypothesis correlation to the GitHub diff of the last deploy on the implicated service; remediation actions staged into recommend / propose / auto tiers with one-click Slack rollback and bounded thread Q&A; deterministic abstention rules (cold start, low confidence, budget exhausted, conflicting leads); replay command re-runs the pipeline on stored incidents and fails CI on regression; typed memory graph with `local` / `aggregated` / `verified` trust tiers; negative user reactions become eval regression fixtures; per-LLM-call traces and CI-fail-on-drift eval harness. Multi-tenant, schema-validated. *Python · FastAPI · Next.js · Anthropic API*
...
- **[winning-writing](https://github.com/kalyvask/winning-writing)** — Agentic writing coach from Stanford GSB's *Winning Writing* (Glenn Kramon) and Rachel Konrad's cold-outreach lectures. **25 Claude skills, 18 principle groups (100+ rules)** spanning the cold-outreach pipeline (recipient research, warm-intro finder, graveyard historian, connection finder, fun-angle, tell-them-something-new), surgical edits (em-dash killer, adverb killer, jargon scrub, be-specific, headline-as-claim, humanize, warmth-and-competence), pitch artifacts (Constine 15-question memo, show-don't-tell), and self-evolving voice files (voice-commit, voice-consolidator). Three execution paths: a Claude Code skill set you install once and forget, an offline browser critic that scores any draft against the rubric, and a **three-mode agentic Coach** — single-shot, single-shot + Haiku-planner-routed polish, or full per-step pipeline (researcher → connector → drafter → parallel surgical edits → rubric) with streaming trace and per-step model choice. Cross-model gate runs a second Opus call to independently approve before send. *JavaScript · Anthropic SDK (web_search, server tools, multi-model) · Claude Skills · OKLCH design tokens*
