...
- **[winning-writing](https://github.com/kalyvask/winning-writing)** — Agentic writing coach from Stanford GSB's *Winning Writing* (Glenn Kramon) and Rachel Konrad's cold-outreach lectures. **25 Claude skills, 18 principle groups (100+ rules)** covering recipient research, warm-intro finding, the graveyard of failed precedents, connection-mining, surgical edits (em-dash killer, jargon scrub, adverb killer, humanize, warmth-and-competence), and pitch artifacts (Constine's 15-question memo, show-don't-tell). Three execution paths: install as Claude Code skills, run an offline browser critic, or use the **three-mode agentic Coach** — single-shot, Haiku-planner-routed polish, or full per-step pipeline (researcher → connector → drafter → parallel surgical edits → rubric) with streaming trace and per-step model choice. Cross-model gate: a second Opus call independently approves every send. *JavaScript · Anthropic SDK (web_search, multi-model) · Claude Skills · OKLCH design tokens*
