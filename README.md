...
- **[role-radar](https://github.com/kalyvask/role-radar)**: PM job matcher for AI companies with one-click interview prep and one-click company review per job. Pulls live roles from ~120 AI companies and VC-backed startups via Greenhouse / Lever / Ashby / SmartRecruiters / Workday connectors, scores each 0–100 against your CV across title/seniority/skills/domains/location, and serves a Flask review UI that learns from like/dislike feedback. Per-job prep button (Claude Opus 4.7) produces a report on the company, the role, and the likely interview questions, then maps your stories to what the role actually wants; second-pass critic scores it 1–10 with severity-tagged findings; styled HTML view plus Word file. Per-company review button (Claude Sonnet 4.5 + `web_search` tool, 15–25 queries) emits an investor-grade analysis with valuation timeline chart, ARR growth, competitor quadrant, funding-rounds table, press / Glassdoor / Reddit sentiment, and a verdict (Strong apply / Apply with caveats / Pass / Inconclusive). Every numeric claim cited inline. Plus an agent layer (`role-radar agent`) that tracks pipeline state across runs and drafts cold outreach per job. *Python · Flask · Typer · Anthropic SDK · Mermaid · python-docx · SQLite*
